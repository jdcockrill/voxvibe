...
class DBusWindowManagerStrategy(WindowManagerStrategy):
    ...
    def store_current_window(self) -> None:
        ...
        if window_id:
            ...
            logger.debug(f"Stored window ID: {self._stored_window_id}")
        else:
    ...
    def focus_and_paste(self, text: str) -> bool:
        ...
        if not self._stored_window_id:
            logger.warning("No stored window_id; cannot focus & paste")
            return False
        ...
        if result:
            logger.debug(f"Successfully pasted text to window {self._stored_window_id}")
        else:
            logger.warning(f"Failed to paste text to window {self._stored_window_id}")
